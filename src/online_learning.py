...
class OnlineLearning():
    ...
    def kalman_filter_initialization(self, PARAMS: dict) -> None:
        ...
        """
        self.kalman_filter = KalmanFilter(self.B, self.Bd, PARAMS)
    ...
    def extract_NN_info(self, NN: torch.nn) -> Tuple[Array, Array]:
        ...
        phi = self.tensor2np(phi_tensor).reshape(-1, 1)
        
        last_layer = NN.fc[-1]
        w_tensor = last_layer.weight.data
        b_tensor = last_layer.bias.data

        w_ = self.tensor2np(w_tensor)
        w = fcs.get_flatten(w_).reshape(-1, 1)
        b = self.tensor2np(b_tensor).reshape(-1, 1)
    ...
    def assign_last_layer(self, NN: torch.nn, value: Array) -> None:
        ...
        w = fcs.get_unflatten(value[0:8800].flatten(), 550)
        b = value[8800:].flatten()
